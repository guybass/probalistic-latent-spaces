...
\begin{enumerate}[leftmargin=*,itemsep=0.35em]
    ...
    \item A connection-selective boundary result: square-root regularity removes explicit vocabulary-size and minimum-probability factors from Levi--Civita bounds, but a Bernoulli family proves that it cannot control any fixed nonzero-$\alpha$ connection without an additional raised-third-moment assumption.
...
\end{equation}
Thus the missing $B_2$ envelope is measurable as a supremum of fourth score moments along the declared chart. Proposition~\ref{prop:score-moments} separately supplies the third moment needed by nonzero-$\alpha$ connections and the exact cancellation $\Gamma^{(1)}=0$ in natural affine-head coordinates.
...
Let $A=\nabla^{p,\LC}-\nabla^{\widetilde p,\LC}$. Up to the sign convention for $A$, the exact variation-of-connection identity is
\begin{equation}
\begin{split}
    P^{p,\LC}_{1\leftarrow0}
    -P^{\widetilde p,\LC}_{1\leftarrow0}
    =-\int_0^1
      P^{p,\LC}_{1\leftarrow t}\,
      A_{\gamma(t)}\!\left(
        \dot\gamma(t),
        P^{\widetilde p,\LC}_{t\leftarrow0}\,\cdot
      \right)\dd t.
\end{split}
