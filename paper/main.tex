...
\begin{enumerate}[leftmargin=*,itemsep=0.35em]
    ...
    \item Boundary-robust Levi--Civita stability: the square-root embedding removes explicit vocabulary-size and minimum-probability factors from metric, connection, transport, and curvature bounds.
...
\begin{equation}
    ...
    \le L_\gamma
    \exp\!\left(\frac{B_1B_2}{\lambda}L_\gamma\right)D_{\LC}.
    \label{eq:sqrt-transport-bound}
\end{equation}
Every displayed constant is independent of $N$ and of $\min_a p_a,\min_a\widetilde p_a$.
\end{theorem}
...
\end{equation}
and each lower-index coefficient has norm at most $B_1B_2$. Raising the final index yields \eqref{eq:sqrt-connection-bound}--\eqref{eq:dlc}. The same identities give $\|\Gamma^{\LC}\|_{\op}\le B_1B_2/\lambda$. Duhamel's formula and Gronwall's inequality along $\gamma$ then give \eqref{eq:sqrt-transport-bound}.
\end{proof}
...
\end{equation}
Thus square-root $C^2$ control alone determines Levi--Civita stability; a general nonzero-$\alpha$ connection additionally requires stability of the third score moment. Affine softmax structure supplies this moment directly through Proposition~\ref{prop:score-moments}.
\end{corollary}
...
Softmax probabilities are positive but can be extremely small. Uniform tokenwise floors are mathematically clean and practically strong, and the general-$\alpha$ probability-coordinate bound is not a useful finite-scale certificate when its constants are dominated by that floor. The square-root Levi--Civita theorem below removes this explicit dependence, and affine heads admit score-moment formulas, but Fisher rank loss remains a genuine instability. Quotienting, coarse-graining, or a preregistered horizontal-subspace analysis is necessary near rank loss; an unreported ridge changes the geometry.
